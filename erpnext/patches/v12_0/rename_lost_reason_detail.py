...
def execute():
    if frappe.db.exists("DocType", "Lost Reason Detail"):
        ...
        frappe.db.sql(
            """INSERT INTO `tabQuotation Lost Reason Detail` 
        ...
        frappe.db.sql(
            """INSERT INTO `tabQuotation Lost Reason` 
            (        `name`,   `creation`,   `modified`,   `modified_by`,   `owner`,   `docstatus`,   `parent`,    `parentfield`,  `parenttype`,   `idx`,   `_comments`,   `_assign`,   `_user_tags`,   `_liked_by`,   `order_lost_reason`)
