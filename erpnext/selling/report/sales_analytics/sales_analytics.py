...
class Analytics(object):
    ...
    def get_data(self):
        ...
        elif self.filters.tree_type == 'Sales Partner':
            self.get_sales_transactions_based_on_sales_partner()
            self.get_rows_by_group()
    ...
    def get_sales_transactions_based_on_sales_partner(self):
        if self.filters["value_quantity"] == 'Value':
            value_field = "base_net_total"
        else:
            value_field = "total_qty"
        ...
        self.entries = frappe.db.sql("""
            SELECT 
                IFNULL(`s`.`sales_partner`, "Sales Partner") AS `entity`, 
                `s`.`{value_field}` AS `value_field`, 
                `s`.`{date_field}`
            FROM `tab{doctype}` AS `s` 
            WHERE 
                `s`.`docstatus` = 1 
                AND `s`.company = "{company}" 
                AND `s`.`{date_field}` BETWEEN "{from_date}" AND "{to_date}"
                /* AND IFNULL(`s`.`sales_partner`, '') != ''  */
            ORDER BY `s`.`sales_partner`
        """.format(
    ...
    def get_sales_partners(self):
        ...
        self.group_entries = frappe.db.sql("""
            ...
            FROM (
                SELECT 
                    "Sales Partner" AS `name`, 
                    0 AS `lft`,
                    2 AS `rgt`, 
                    '' AS `parent`
                UNION SELECT 
                    DISTINCT `sales_partner` AS `name`, 
                    1 AS `lft`, 
                    1 AS rgt, 
                    "Sales Partner" AS `parent`
                FROM `tab{doctype}` 
                WHERE IFNULL(`sales_partner`, '') != ''
            ) AS `b`
            ORDER BY `lft`, `name`;
        """
        .format(doctype=self.filters.doc_type), as_dict=True)
