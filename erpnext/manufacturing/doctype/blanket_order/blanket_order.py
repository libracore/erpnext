...
class BlanketOrder(Document):
    def update_ordered_qty(self):
        if self.blanket_order_type == "Selling":
            ...
            item_delivered_qty = frappe._dict(frappe.db.sql("""
                select trans_item.item_code, sum(trans_item.delivered_qty) as qty
                from `tab{0} Item` trans_item, `tab{0}` trans
                where trans.name = trans_item.parent
                    and trans_item.blanket_order=%s
                    and trans.docstatus=1
            ...
            """.format(ref_doctype), self.name))
            for d in self.items:
                if item_ordered_qty.get(d.item_code, 0) < item_delivered_qty.get(d.item_code, 0):
                    d.db_set("ordered_qty", item_delivered_qty.get(d.item_code, 0))
                else:
